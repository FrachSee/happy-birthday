...
- Continuous party FX: `script/party.js` runs the whole time, spawning floating emoji on a `#party-layer` (fixed, `pointer-events:none`, `z-index:0`, so `.container` is `z-index:1`) and periodic confetti side-bursts via its OWN `confetti.create()` canvas (kept separate from `main.js`'s foreground bursts so z-index never clashes). It honors `prefers-reduced-motion` and pauses on `visibilitychange`.
- Fonts use `clamp()` so they scale with the viewport; adjust the min/max bounds rather than reintroducing fixed per-breakpoint font sizes.
- Admin music controls persist in two places: settings (`volume`, `autoplay`) in `localStorage.hbdAudio`, and a replaced track as a `Blob` in IndexedDB (db `hbd-audio`, store `tracks`, key `bgm`). `script/audio.js` reads both; a per-user `localStorage.hbdMuted` toggle always overrides the admin autoplay default. A replaced track is device-local (no static export) — to publish a new song for everyone, overwrite `music/bgm.mp3`.

### Testing preference
- The maintainer runs manual/visual (GUI) testing themselves. Prefer keeping automated checks lightweight (e.g. `node --check` on the JS, quick static serving checks) and avoid lengthy live computer-use walkthroughs unless explicitly requested.
